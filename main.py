...
temp_read = "/sys/class/thermal/thermal_zone0/temp"
...
class App(ctk.CTk):
    ...
    def display(self):
        ...
        CTkButton(button_canvas, text="CPU Temp.", command=self.temp).grid(row=12, column=0, padx=10, pady=10)
        self.temp_text = self.screen_canvas.create_text(80, 640, text=" ", font=("Courier", 12))
    ...
    def temp(self):
        ...
        plt.xlabel("Time per 10 seconds")
        plt.ylabel("Temperature")
        plt.title("WSOne CPU temperature check")
        # Other stuff
        self.ax.grid()
        for x in np.arange(0, x_time, 0.1):
            temp = subprocess.getoutput(ssh + " " + root + self.ip_address.get() + " " + cat + " " + temp_read)
            self.real_temp = int(temp) / 1000
            self.screen_canvas.itemconfig(self.temp_text, text=f"{self.real_temp} \xb0C")
            xdata.append(x)
            ydata.append(self.real_temp)
